...
class SRSolver(BaseSolver):
    ...
    def feed_data(self, batch, need_HR=True):
        input = batch['LR']
        input_ = batch['PAN']
        # if self.opt['networks']['which_model'] == 'BIEDN': #or self.opt['networks']['which_model'] == 'BDPN':
        #     lrpan = batch['LRPAN']
        #     self.LRPAN.resize_(lrpan.size()).copy_(lrpan)
    ...
    def train_step(self, mask=None, lamda=0):
        self.model.train()
        self.optimizer.zero_grad()
        if 'mask' in self.opt['networks']['which_model']:
            output = self.model(self.LR, self.PAN, mask)
            loss = self.criterion_pix(output, self.HR, mask)
        elif 'BDPN' in self.opt['networks']['which_model']:
            output = self.model(self.LR, self.PAN)
            loss = self.criterion_pix(output, self.LRPAN, self.HR, lamda)
    ...
    def _forward_x8(self, x, pan, forward_function, mask=None):
        """
        self ensemble
        """
        def _transform(v, op):
